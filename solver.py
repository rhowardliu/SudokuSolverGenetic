...
def getSolvableBox(puzzle):
  for box in puzzle.boxes:
    ...
    taken_values = set()
    taken_values.add(other_box.val for other_box in puzzle.horizontal[box.i].boxes)
    taken_values.add(other_box.val for other_box in puzzle.vertical[box.j].boxes)
    taken_values.add(other_box.val for other_box in puzzle.bigbox[puzzle.findBoxNumber(box.i, box.j)].boxes)
    print("Taken ", taken_values)
    possible_values = possible_values - taken_values
...
def getIffyBoxes(puzzle):
  ...
  for box in boxes:
    ...
    possible_values.remove(box.val for box in puzzle.bigbox[puzzle.findBoxNumber(box.i, box.j)].boxes)
